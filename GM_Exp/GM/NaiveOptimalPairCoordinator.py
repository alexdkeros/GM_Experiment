...
class NaiveOptimalPairCoordinator(Coordinator):
    '''
    ...
    def __init__(self,  env, nodes, 
        ...
        self.pairType=1
        self.nodeRequestFlag=False #flag discriminates between report due to LV (no imediate Balancing) and request during bal process
    ...
    def rep(self,dat,sender):
        ...
        '''
        #DBG
        print("adding to balset node: %s"%str(sender))
        self.balancingSet.add((sender,)+dat) 
        self.remainingReps-=1
        if self.remainingReps<=0 and self.nodeRequestFlag:   
            self.balance()
    ...
    def balance(self):
        ...
        if self.monitoringFunction(b)<self.threshold:
            ...
            if len(diffSet): #i.e. len(balancingSet)!=len(nodes)
                reqNodeId=self.optimalPairer.getOptPairingOfTypeFromSet(self.pairType,set(i for i,v,u,vel in self.balancingSet))
                self.pairType*=2
